...
class CustomDropdownState<T> extends State<CustomDropdown<T>>
  ...
  void initState() {
    super.initState();
    if (widget.child == null && widget.currentIndex == -1) widget.currentIndex = 0;
  ...
  Widget build(BuildContext context) {
    final DropdownButtonStyle style = widget.dropdownButtonStyle;
            ...
            children: [
              if (widget.currentIndex == -1) ...[
                Expanded(
                  child: DropdownItem(
                    value: -1,
                    child: Opacity(opacity: 1, child: widget.child),
                  ),
                ),
              ] else ...[
                Expanded(child: widget.items[widget.currentIndex]),
              ],
              if (!widget.hideIcon)
                RotationTransition(
                  ...
                  child: Padding(
                    padding:
                    widget.paddingIcon ?? const EdgeInsets.symmetric(horizontal: 10),
                    child: widget.icon ??
                        const Icon(
                      ...
                      child: ConstrainedBox(
                        ...
                        child: ListView(
                          ...
                          children: widget.items.asMap().entries.map((item) {
                            return InkWell(
                              onTap: () {
                                setState(() => widget.currentIndex = item.key);
                              ...
                              child: Column(
                                ...
                                children: [
                                  Opacity(
                                    // opacity: (widget.currentIndex == item.key) ? 1 : 0.2,
                                    opacity: 1,
                                    child: item.value,
                                  ),
                                  const Divider(
